...
export function useColumnsByCategory(
  ...
  const commonColumns = useCommonLogsColumns(isAdmin, canViewRequestAudit)
  const drawingColumns = useDrawingLogsColumns(isAdmin, canViewRequestAudit)
  const taskColumns = useTaskLogsColumns(isAdmin, canViewRequestAudit)
